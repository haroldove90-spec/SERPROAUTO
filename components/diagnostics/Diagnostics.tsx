...
const Diagnostics: React.FC<DiagnosticsProps> = ({ vehicles, onUpdateVehicle }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [activeTab, setActiveTab] = useState('general');
  const [formData, setFormData] = useState<Vehicle | null>(null);

  const diagnosticVehicles = useMemo(() => {
    return vehicles.filter(v => v.status === VehicleStatus.InDiagnosis);
  }, [vehicles]);
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-bold text-kia-light">Diagnósticos Pendientes</h2>
      <p className="text-kia-gray">Vehículos que requieren revisión y diagnóstico por parte de un técnico.</p>
      
      <VehicleTable vehicles={diagnosticVehicles} onEdit={handleOpenModal} showAllColumns={false} />

      {isModalOpen && formData && (
        <Modal onClose={handleCloseModal} title="Detalles de Diagnóstico">
